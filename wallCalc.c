...
/*   Updated: 2023/04/09 05:15:00 by adaifi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	orizontios_hit(t_data *data, double f_ray)
{
	...
	while (data->gridhor_x <= data->width && \
	data->gridhor_x > 0 && data->gridhor_y <= \
	data->height && data->gridhor_y > 0)
	{
...
void	sichoucu_hit(t_data *data, double deg)
{
	...
	while (data->gridver_x <= data->width && \
	data->gridver_x > 0 && data->gridver_y <= \
	data->height && data->gridver_y > 0)
	{
		i = (int)floor(data->gridver_y / (double)TILE) + data->map->index;
		j = (int)floor(data->gridver_x / (double)TILE);
		if (check_ver_hit(data, i, j, deg) == true)
			return ;
